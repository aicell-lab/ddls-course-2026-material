...
| Value | Count | Meaning |
|---|---|---|
| `ok` | 45,483 | Nothing flagged. |
| `edge_evaporation` | 11,356 | Well is on the plate perimeter (row A or P, column 1 or 24). These wells lose volume across the incubation and the medium concentrates. |
| `low_volume` | 499 | Dispenser under-delivered into this well. |
| `read_error` | 262 | The reader failed on this well. `growth_od600` and `resazurin_rfu` are empty. |
...
| `mic90_um` | float | µM | Concentration inhibiting 90% of growth. **Empty for 883 of 1,800 compounds — those did not reach 90% inhibition at the highest tested concentration (128 µM).** Missing means "not active up to 128 µM", not "not measured". |
...
We are telling you these up front because we know about them. There may be
others we do not.
...
3. **Edge wells evaporate.** Perimeter wells are flagged `edge_evaporation`.
   They are about 20% of every plate.
